...
def _list_orders():
    """Fetch orders and calculate student counts for the dashboard"""
    try:
        ...
        order_list = []
        for o in orders:
            o['student_count'] = o.pop('total_students')
            order_list.append(o)

        return JsonResponse(order_list, safe=False)
        
    except Exception as e:
        print(f"CRITICAL ERROR in _list_orders: {str(e)}")
...
def _create_order(request):
    try:
        ...
        if not school_name or not batch_name:
            return JsonResponse({'error': 'Missing school/batch name'}, status=400)

        if not isinstance(students, list) or len(students) == 0:
            return JsonResponse({'error': 'Student list is empty'}, status=400)
...
def start_processing(request, pk):
    try:
        ...
        thread.daemon = True
        thread.start()
        return JsonResponse({'message': 'Processing started in the background', 'order_id': pk})
    except Exception as e:
        return JsonResponse({'error': str(e)}, status=500)
